...
export function movePawn({
    ...
    const figureOnEndCell = cells.find((item) => item.id === endCellId).figure;
    const isAnAttack =
        figureOnEndCell &&
        ...
        (color === WHITE
            ...
            : grabbedFigure.id + 9 === endCellId || grabbedFigure.id + 7 === endCellId);
    const isOneCellMove = color === WHITE ? grabbedFigure.id - 8 : grabbedFigure.id + 8;
    ...
    if (figureOnEndCell) {
        if (isAnAttack) {
            setBoard(cells, setCells, figureToLand, grabbedFigure, setNextMove, coordinates, setCoordinates, endCellId);
            setTakenFigures({
                ...takenFigures,
        ...
        (isTwoCellsMove &&
            ...
            !cells.find((item) => item.id === isOneCellMove).figure)
    ) {
        setBoard(cells, setCells, figureToLand, grabbedFigure, setNextMove, coordinates, setCoordinates, endCellId);
    } else {
        alert("Wrong move");
